...
class HDU:
    ...
    def login(self):
        ...
        try:
            ...
            req = request.Request(HDU.URL_LOGIN, post_data, HDU.headers)
            response = self.opener.open(req).read().decode()
    ...
    def submit(self, problem_id, language, src_code):
        ...
        try:
            req = request.Request(HDU.URL_SUBMIT, post_data, HDU.headers)
    ...
    def result(self):
        ...
        url = HDU.URL_STATUS + parse.urlencode(data)

        try:
            req = request.Request(url, None, HDU.headers)
            page = self.opener.open(req, timeout=5)
...
def hdu_submit(problem_id, language_name, src_code, url=None, token=None, sid=None):
    with open(os.path.join(pathlib.Path(__file__).parent, 'hdu.json')) as fp:
